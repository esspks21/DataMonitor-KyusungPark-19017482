...
이 프로젝트는 **Proof of Concept(PoC)** 로, 아래 핵심 가능성을 검증하기 위해 작성되었습니다.
...
|---|---|
| 주제 | 반도체 시료 생산주문관리 |
| 도구 형태 | 콘솔 기반 관리자 실시간 조회 도구 |
| 데이터 소스 | JSON 파일 (외부 DB 없이 파일 단독 운용) |
| 구현 언어 | C++20 (외부 라이브러리 의존성 Zero) |

**PoC 범위**

- JSON 파일을 데이터 소스로 삼아 주문 데이터를 런타임에 로드·파싱하는 구조 검증
- 콘솔 UI만으로 필터링·정렬·통계 등 관리자 조회 시나리오 전체를 커버할 수 있는지 검증
- 파일 재로드(새로고침) 방식으로 외부 데이터 변경을 반영하는 핫 리로드 패턴 검증
- ANSI 색상 코드를 활용한 상태·우선순위 시각화 가능성 검증

**PoC 제외 범위**

- 실시간 DB 연동 (MES, ERP 시스템 연계)
- 주문 데이터 생성·수정·삭제 (현재는 읽기 전용)
- 네트워크 통신, 멀티스레드 자동 갱신
